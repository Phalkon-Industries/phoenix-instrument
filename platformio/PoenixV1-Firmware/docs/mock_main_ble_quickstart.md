...
| Command characteristic UUID | `c1883ec3-d984-4dcd-9d67-41ade54c5f2a` (`k_mock_ble_command_characteristic_uuid`), write-without-response |
| Notification characteristic UUID | `cd1411cc-4ab8-46e8-9ad5-c41a871caf41` (`k_mock_ble_notification_characteristic_uuid`), notify |
| Connection interval | 20–40 ms preferred |
...
All commands are written as UTF-8 JSON payloads to the command characteristic. Each payload contains a `command` field and optional arguments. Responses arrive as JSON packets on the notification characteristic, wrapped in the standard Phoenix envelope generated by `phoenix_ble_pack_notification_message` (for example `{"command":"reference_start","parameters":{...}}`). The `parameters` block mirrors the field names asserted in the Unity tests.
