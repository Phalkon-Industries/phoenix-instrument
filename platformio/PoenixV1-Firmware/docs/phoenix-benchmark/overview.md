...
- Channel-map summary tables list each state (and the overall cycle) with aligned columns: samples, channel statistics (mean/std/min/max), timing metrics, the channel alignment verdict, and a `Warnings` column sourced from the saturation detector. The CLI parses this table to populate the exported JSON and the channel dominance plot.
