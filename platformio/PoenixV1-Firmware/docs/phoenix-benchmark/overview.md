...
- Firmware idles after boot, waits for either a `channel_map` or `adc_speed` command, executes the requested scenario, and then returns to a fresh `# ready` prompt so hosts can pipeline multiple operations.
...
- The Python CLI runs the plan, captures the serial transcript, and produces a Markdown report that bundles the raw summary table, a JSON export, and scenario-aware plots. Mixed plans (channel map + ADC throughput) appear in a single report with per-scenario tables.
...
2. **Dry-run Plans** – Use the Python CLI (within the shared conda environment) to validate command plans without touching hardware. Sample plans now cover both individual and combined scenarios (`docs/phoenix-benchmark/sample_plans/channel_map_phase1.json` and `docs/phoenix-benchmark/sample_plans/combined_channel_map_adc_speed.json`):
   ```powershell
   ...
   The combined template demonstrates issuing a `channel_map` sweep followed by an `adc_speed` throughput run.
...
4. **Automated Report** – After each scenario signals `# benchmark_complete`, the CLI writes a report bundle (`transcript_<scenarios>.txt`, `summary_<scenarios>.json`, `report.md`, plus scenario-specific plots) to the chosen output folder (default: `~/Downloads/phoenix-benchmark/<timestamp>`). The Markdown file embeds every summary table so lab staff can review mixed runs without cross-referencing multiple files.
...
For reference, `docs/phoenix-benchmark/sample_logs/combined_run_transcript.txt` captures a full two-command session including the metadata banners and both summary formats.
