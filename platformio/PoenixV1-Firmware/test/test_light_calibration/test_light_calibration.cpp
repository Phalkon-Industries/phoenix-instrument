...
static int32_t  g_test_blue_max_by_wiper[1024];
static int32_t  g_test_green_max_by_wiper[1024];
...
static void reset_test_state(void) {
  ...
  // Default: all wiper codes report saturating ADC values so tests that
  // need below-threshold behaviour must explicitly override the relevant range.
...
static void test_calibration_finds_optimal_below_threshold(void) {
  ...
  // Indices 256-1023 saturate (set by reset_test_state), so only 0-255 are in play.
...
static void test_calibration_handles_all_saturated(void) {
  ...
  // Step 2: Set all ADC codes above saturation threshold (indices 256-1023 are
  // already saturated from reset_test_state, but override 0-255 explicitly).
  const int32_t threshold = light_calibration_saturation_threshold();
  for (int i = 0; i < 256; ++i) {
    g_test_blue_max_by_wiper[i]  = threshold + 100000;
    g_test_green_max_by_wiper[i] = threshold + 100000;
...
static void test_calibration_reports_per_channel_results(void) {
  ...
  // Indices 256-1023 are already saturated from reset_test_state.
  const int32_t threshold = light_calibration_saturation_threshold();
  for (int i = 0; i < 256; ++i) {
...
static void test_calibration_null_config_uses_defaults(void) {
  ...
  // Step 3: Verify it ran successfully with defaults (1024 wiper values, 0-1023).
  TEST_ASSERT_TRUE(result.success);
  TEST_ASSERT_EQUAL_UINT32(1024u, g_test_sweep_call_count);
