...
using phoenix_benchmark_support::is_adc_code_saturated;
...
static void test_determine_dominant_channel_prefers_channel_a_when_delta_exceeds_drain(void) {
  StateAccumulator drain_accumulator;
  ...
  StateAccumulator led1_accumulator;
  ...
  const BenchmarkChannel channel = determine_dominant_channel(drain_accumulator, led1_accumulator, 5.0);
...
static void test_determine_dominant_channel_returns_unknown_when_deltas_similar(void) {
  ...
  StateAccumulator led2_accumulator;
  ...
  const BenchmarkChannel channel = determine_dominant_channel(drain_accumulator, led2_accumulator, 5.0);
...
static void test_is_adc_code_saturated_detects_full_scale_codes(void) {
  TEST_ASSERT_TRUE(is_adc_code_saturated(8388607));
  TEST_ASSERT_TRUE(is_adc_code_saturated(-8388608));
  TEST_ASSERT_FALSE(is_adc_code_saturated(0));
  TEST_ASSERT_FALSE(is_adc_code_saturated(1024));
}
