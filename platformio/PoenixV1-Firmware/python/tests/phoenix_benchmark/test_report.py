...
def _drift_capture_lines() -> List[str]:
    return [
        ...
        "# drift_capture,metadata,start_us=0,end_us=50,step_delay_us=10,osr=4096,wiper_blue=0x2A,wiper_green=0x54",
...
def test_create_report_handles_drift_capture(tmp_path: Path) -> None:
    ...
    assert data["metadata"]["wiper_blue"] == "0x2A"
    assert data["metadata"]["wiper_green"] == "0x54"
    ...
    assert (
        "| 1 | 0 | 50 | 10 | 4096 | 0x2A | 0x54 | 3 | 2 | buffer_overflow, saturation |"
        in report_text
    )
