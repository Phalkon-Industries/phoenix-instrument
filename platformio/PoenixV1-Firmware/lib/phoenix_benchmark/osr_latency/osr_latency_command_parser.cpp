...
bool parse_parameters_object(const char** cursor, PhoenixBenchmarkOsrLatencyOptions* options, const char** error) {
  if ((cursor == nullptr) || (*cursor == nullptr) || (options == nullptr)) {
    if (error != nullptr) {
      *error = k_phoenix_benchmark_osr_latency_error_invalid_command;
    }
    return false;
  }
  ...
  while (true) {
    ...
    *cursor = skip_whitespace(*cursor);
    ...
    if ((std::strcmp(key_buffer, "samples") == 0) || (std::strcmp(key_buffer, "sample_count") == 0)) {
      uint32_t parsed_value = 0u;
      if (!parse_unsigned_value(*cursor, 10, &parsed_value, cursor) || (parsed_value == 0u)) {
        if (error != nullptr) {
          *error = k_phoenix_benchmark_osr_latency_error_invalid_value;
        }
        return false;
      }
      options->sample_count        = parsed_value;
      options->has_sample_override = true;
    }
    else if ((std::strcmp(key_buffer, "warmup") == 0) || (std::strcmp(key_buffer, "warmup_count") == 0)) {
      uint32_t parsed_value = 0u;
      if (!parse_unsigned_value(*cursor, 10, &parsed_value, cursor)) {
        if (error != nullptr) {
          *error = k_phoenix_benchmark_osr_latency_error_invalid_value;
        }
        return false;
      }
      options->warmup_count        = parsed_value;
      options->has_warmup_override = true;
    }
    else if (std::strcmp(key_buffer, "include_blocking") == 0) {
      ...
      options->include_blocking      = flag_value;
      options->has_blocking_override = true;
    }
    else if (std::strcmp(key_buffer, "include_irq") == 0) {
      bool flag_value = true;
      if (!parse_boolean_value(*cursor, &flag_value, cursor)) {
        if (error != nullptr) {
          *error = k_phoenix_benchmark_osr_latency_error_invalid_value;
      ...
      options->include_irq      = flag_value;
...
bool handle_key_value(const char* key_buffer, const char** cursor, PhoenixBenchmarkOsrLatencyOptions* options,
                      const char** error) {
  ...
  if ((std::strcmp(key_buffer, "warmup") == 0) || (std::strcmp(key_buffer, "warmup_count") == 0)) {
    uint32_t parsed_value = 0u;
    if (!parse_unsigned_value(*cursor, 10, &parsed_value, cursor)) {
      if (error != nullptr) {
        *error = k_phoenix_benchmark_osr_latency_error_invalid_value;
  ...
  if (std::strcmp(key_buffer, "include_blocking") == 0) {
    bool flag_value = true;
    if (!parse_boolean_value(*cursor, &flag_value, cursor)) {
      if (error != nullptr) {
        *error = k_phoenix_benchmark_osr_latency_error_invalid_value;
  ...
  if (std::strcmp(key_buffer, "include_irq") == 0) {
    bool flag_value = true;
    if (!parse_boolean_value(*cursor, &flag_value, cursor)) {
      if (error != nullptr) {
        *error = k_phoenix_benchmark_osr_latency_error_invalid_value;
...
bool parse_plain_token(const char* token) {
  if (token == nullptr) {
    return false;
  }
  ...
  if (std::strncmp(cursor, "osr_latency", 11) != 0) {
    return false;
  }
  cursor += 11;
  cursor = skip_whitespace(cursor);
...
PhoenixBenchmarkOsrLatencyParseOutcome phoenix_benchmark_osr_latency_parse_command_line(const char* line) {
  ...
  while (true) {
    ...
    if (std::strcmp(key_buffer, "command") == 0) {
      ...
      if (!strings_equal(value_start, value_length, "osr_latency")) {
        return {false, options, k_phoenix_benchmark_osr_latency_error_invalid_command};
      }
